...
def mock_db(mocker):
    ...
    db.get_cve_guild_config.return_value = {'enabled': True, 'severity_threshold': 'all', 'verbose_mode': False}
    db.get_cve_channel_config.return_value = {'enabled': True, 'verbose_mode': None} # Channel enabled, no verbosity override
...
def mock_cve_monitor(mocker):
    ...
    monitor.find_cves.side_effect = lambda content: CVEMonitor.CVE_REGEX.findall(content) # Use real find_cves
    monitor.get_cve_data = AsyncMock(return_value=None) # Default: No data found
    monitor.check_severity_threshold.return_value = (True, "High") # Default: Passes threshold
    monitor.create_cve_embed = MagicMock(return_value=discord.Embed(title="Mock CVE Embed"))
    monitor.check_kev.return_value = None # Default: Not in KEV
    monitor.create_kev_status_embed = MagicMock(return_value=discord.Embed(title="Mock KEV Embed"))
    return monitor
...
def mock_bot(mocker, mock_db, mock_cve_monitor):
    ...
    mocker.patch.object(SecurityBot, 'user', new_callable=PropertyMock, return_value=mock_user)
...
def mock_bot_with_tasks(mocker, mock_db, mock_cve_monitor, mock_cisa_kev_client):
    ...
    mocker.patch.object(SecurityBot, 'latency', new_callable=PropertyMock, return_value=0.12345)
    ...
    bot._create_kev_embed = MagicMock(return_value=discord.Embed(title="Mock KEV Task Embed"))
...
async def test_on_message_ignore_bots(mock_bot, mock_message):
    ...
    assert mock_bot.stats_messages_processed == 0 # Should not increment if ignored early
...
async def test_on_message_guild_disabled(mock_bot, mock_message, mock_db):
    ...
    mock_message.content = "CVE-2023-1234"
    mock_db.get_cve_guild_config.return_value = {'enabled': False} # Override default mock
...
@pytest.mark.asyncio
async def test_on_message_success_simple_cve_non_verbose_no_kev(mock_bot, mock_message, mock_db, mock_cve_monitor):
    ...
    mock_cve_monitor.check_severity_threshold.return_value = (True, "High") # Passes threshold
    ...
    mock_db.get_cve_guild_config.assert_called_once_with(mock_message.guild.id)
    mock_db.get_cve_channel_config.assert_called_once_with(mock_message.guild.id, mock_message.channel.id)
    mock_db.get_effective_verbosity.assert_called_once_with(mock_message.guild.id, mock_message.channel.id)
    
    # Monitor Calls
    mock_cve_monitor.get_cve_data.assert_awaited_once_with(cve_id_upper)
    mock_cve_monitor.check_severity_threshold.assert_called_once_with(mock_cve_data, 'all')
    mock_cve_monitor.create_cve_embed.assert_called_once_with(mock_cve_data, verbose=False)
    ...
    # Discord Calls
    mock_message.channel.send.assert_awaited_once_with(embed=mock_cve_monitor.create_cve_embed.return_value)
...
@pytest.mark.asyncio
async def test_on_message_success_verbose_global(mock_bot, mock_message, mock_db, mock_cve_monitor):
    ...
    # --- Mock Setup ---
    mock_db.get_cve_guild_config.return_value['verbose_mode'] = True # Global verbose TRUE
    ...
    # --- Assertions ---
    mock_db.get_effective_verbosity.assert_called_once_with(mock_message.guild.id, mock_message.channel.id)
    mock_cve_monitor.create_cve_embed.assert_called_once_with(mock_cve_data, verbose=True) # Check verbose=True
    mock_message.channel.send.assert_awaited_once_with(embed=mock_cve_monitor.create_cve_embed.return_value)
    mock_cve_monitor.check_kev.assert_awaited_once_with(cve_id_upper)
    mock_cve_monitor.create_kev_status_embed.assert_not_called()

@pytest.mark.asyncio
async def test_on_message_success_channel_override_verbose(mock_bot, mock_message, mock_db, mock_cve_monitor):
    ...
    mock_db.get_cve_channel_config.return_value = {'enabled': True, 'verbose_mode': True} 
    mock_db.get_effective_verbosity.return_value = True # Effective is TRUE due to channel
    mock_cve_monitor.get_cve_data.return_value = mock_cve_data
    mock_cve_monitor.check_kev.return_value = None # No KEV
...
@pytest.mark.asyncio
async def test_on_message_success_with_kev_non_verbose(mock_bot, mock_message, mock_db, mock_cve_monitor):
    ...
    mock_cve_monitor.check_kev.assert_awaited_once_with(cve_id_upper)
    mock_cve_monitor.create_cve_embed.assert_called_once_with(mock_cve_data, verbose=False)
    mock_cve_monitor.create_kev_status_embed.assert_called_once_with(cve_id_upper, mock_kev_data, verbose=False) # KEV embed non-verbose
    ...
    assert send_calls[1] == call(embed=mock_cve_monitor.create_kev_status_embed.return_value)
...
@pytest.mark.asyncio
async def test_on_message_success_with_kev_verbose(mock_bot, mock_message, mock_db, mock_cve_monitor):
    ...
    # --- Assertions ---
    mock_cve_monitor.check_kev.assert_awaited_once_with(cve_id_upper)
    mock_cve_monitor.create_cve_embed.assert_called_once_with(mock_cve_data, verbose=True) # Verbose
    mock_cve_monitor.create_kev_status_embed.assert_called_once_with(cve_id_upper, mock_kev_data, verbose=True) # KEV embed verbose
...
async def test_on_message_cache_hit(mock_bot, mock_message, mock_db, mock_cve_monitor):
    ...
    mock_bot.recently_processed_cves[cache_key] = datetime.now(timezone.utc) - timedelta(seconds=5)
    mock_cve_monitor.get_cve_data.return_value = {'id': cve_id_upper} # Need some data for first pass if cache fails
...
@pytest.mark.asyncio
async def test_on_message_severity_threshold_fail(mock_bot, mock_message, mock_db, mock_cve_monitor):
    ...
    # --- Assertions ---
    mock_cve_monitor.get_cve_data.assert_awaited_once_with(cve_id_upper)
    mock_cve_monitor.check_severity_threshold.assert_called_once_with(mock_cve_data, 'medium')
...
@pytest.mark.asyncio
async def test_on_message_multiple_cves_below_limit(mock_bot, mock_message, mock_db, mock_cve_monitor):
    ...
    mock_cve_monitor.create_cve_embed.assert_has_calls([
        call(mock_cve_data1, verbose=False),
        call(mock_cve_data2, verbose=False)
    ])
...
@pytest.mark.asyncio
async def test_on_message_multiple_cves_above_limit(mocker, mock_bot, mock_message, mock_db, mock_cve_monitor):
    ...
    assert mock_cve_monitor.get_cve_data.await_count == 2
    mock_cve_monitor.get_cve_data.assert_has_awaits([call(cve1), call(cve2)]) # No call for cve3
    assert mock_cve_monitor.check_severity_threshold.call_count == 2
    assert mock_cve_monitor.create_cve_embed.call_count == 2
    assert mock_cve_monitor.check_kev.await_count == 2
...
@pytest.mark.asyncio
async def test_on_message_error_nvd_rate_limit(mock_bot, mock_message, mock_db, mock_cve_monitor):
...
@pytest.mark.asyncio
async def test_on_message_error_kev_check(mock_bot, mock_message, mock_db, mock_cve_monitor):
...
@pytest.mark.asyncio
async def test_on_message_error_discord_forbidden(mock_bot, mock_message, mock_db, mock_cve_monitor):
    ...
    mock_message.channel.send.side_effect = discord.Forbidden(MagicMock(), "Missing Permissions")
...
@pytest.mark.asyncio
async def test_on_message_error_discord_http(mock_bot, mock_message, mock_db, mock_cve_monitor):
    ...
    mock_cve_monitor.get_cve_data.return_value = mock_cve_data
    mock_db.get_effective_verbosity.return_value = False
    mock_message.channel.send.side_effect = discord.HTTPException(MagicMock(), "Server Error")
...
@pytest.mark.asyncio
async def test_on_message_error_generic_exception(mock_bot, mock_message, mock_db, mock_cve_monitor):
...
@pytest.mark.asyncio
async def test_check_cisa_kev_feed_no_new_entries(mock_bot_with_tasks, mock_cisa_kev_client, mock_db):
    ...
    start_time = mock_bot_with_tasks.timestamp_last_kev_check_success # Capture initial state
...
@pytest.mark.asyncio
async def test_check_cisa_kev_feed_new_entries_success(mock_bot_with_tasks, mock_cisa_kev_client, mock_db):
    ...
    mock_bot_with_tasks.get_guild.side_effect = lambda gid: mock_guild1 if gid == guild_id1 else mock_guild2
    mock_bot_with_tasks.get_channel.side_effect = lambda cid: mock_channel1 if cid == channel_id1 else mock_channel2
    ...
    mock_bot_with_tasks._create_kev_embed.assert_has_calls([
        call(new_entry1), call(new_entry2), # For first config
        call(new_entry1), call(new_entry2)  # For second config
    ], any_order=True)
...
@pytest.mark.asyncio
async def test_check_cisa_kev_feed_client_error(mock_bot_with_tasks, mock_cisa_kev_client, mock_db):
...
@pytest.mark.asyncio
async def test_check_cisa_kev_feed_discord_forbidden(mock_bot_with_tasks, mock_cisa_kev_client, mock_db):
...
async def test_send_stats_to_webapp_success(mocker, mock_bot_with_tasks, caplog):
    ...
    mock_post_stats = mocker.patch.object(mock_bot_with_tasks, '_post_stats', new_callable=AsyncMock)
    ...
    assert f"Successfully sent stats to web app (Status: 200)" in caplog.text # Check for success log
...
async def test_send_stats_to_webapp_no_url(mocker, mock_bot_with_tasks, caplog):
    ...
    patched_post = mocker.patch('aiohttp.ClientSession.post') # Patch post just to assert not called
...
async def test_send_stats_to_webapp_http_error(mocker, mock_bot_with_tasks, caplog):
    ...
    assert f"Failed to send stats to web app. Status: 500. Response: Server Error Details" in caplog.text
...
@pytest.mark.asyncio
async def test_send_stats_to_webapp_connection_error(mocker, mock_bot_with_tasks, caplog):
    ...
    connection_error = aiohttp.ClientConnectorError(MagicMock(), OSError("DNS lookup failed"))
    ...
    assert f"Sending stats payload to {expected_url}" in caplog.text # Check if the attempt log happened
    # Check the specific error log message structure
    assert f"Connection error sending stats to web app {expected_url}: ClientConnectorError" in caplog.text # Check specific error type

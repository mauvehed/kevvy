...
def test_disable_nonexistent_kev_config(
    kev_db: KEVConfigDB, db_conn: sqlite3.Connection
):
...
def test_get_enabled_kev_configs(kev_db: KEVConfigDB):
    ...
    actual_configs = {
        (config["guild_id"], config["channel_id"]) for config in enabled_configs
    }
...
def test_log_cve_alert_history(kev_db: KEVConfigDB, db_conn: sqlite3.Connection):
    ...
    cursor = db_conn.cursor()
    cursor.execute(
        "SELECT * FROM cve_monitoring_history WHERE guild_id = ? AND channel_id = ? AND cve_id = ?",
...
def test_log_kev_latest_query(kev_db: KEVConfigDB, db_conn: sqlite3.Connection):
    ...
    cursor = db_conn.cursor()
    cursor.execute(
        "SELECT * FROM kev_latest_queries WHERE guild_id = ? AND user_id = ?",
        (guild_id, user_id),
    )
    row = cursor.fetchone()
    assert row is not None
...
def test_set_and_get_cve_guild_config(kev_db: KEVConfigDB, db_conn: sqlite3.Connection):
    ...
    # Set initial config
    kev_db.set_cve_guild_config(
        guild_id, enabled=True, verbose_mode=False, severity_threshold="high"
    )
    ...
    assert not config["verbose_mode"]
    assert config["severity_threshold"] == "high"

    # Update config
    kev_db.set_cve_guild_config(
        guild_id, enabled=True, verbose_mode=True, severity_threshold="critical"
    )
    config_updated = kev_db.get_cve_guild_config(guild_id)
    assert config_updated is not None
    assert config_updated["verbose_mode"]
...
def test_add_and_get_cve_channel_config(kev_db: KEVConfigDB):
    ...
    # Add config
    kev_db.add_or_update_cve_channel(
        guild_id, channel_id, enabled=True, verbose_mode=True, severity_threshold="low"
    )
...
def test_set_all_channel_verbosity(kev_db: KEVConfigDB):
    ...
    config1 = kev_db.get_cve_channel_config(guild_id, channel_id_1)
    config2 = kev_db.get_cve_channel_config(guild_id, channel_id_2)
    assert config1 is not None and config1["verbose_mode"]
    assert config2 is not None and config2["verbose_mode"]

    # Set all to False
    kev_db.set_all_channel_verbosity(guild_id, False)
    config1 = kev_db.get_cve_channel_config(guild_id, channel_id_1)
    config2 = kev_db.get_cve_channel_config(guild_id, channel_id_2)
    assert config1 is not None and not config1["verbose_mode"]
    assert config2 is not None and not config2["verbose_mode"]
...
def test_get_effective_verbosity(kev_db: KEVConfigDB):
    ...
    kev_db.add_or_update_cve_channel(
        guild_id, channel_override_false, verbose_mode=False
    )
    kev_db.add_or_update_cve_channel(guild_id, channel_override_none, verbose_mode=None)
    # No config added for channel_no_config

    assert kev_db.get_effective_verbosity(guild_id, channel_override_true) is True
    assert kev_db.get_effective_verbosity(guild_id, channel_override_false) is False
    assert (
        kev_db.get_effective_verbosity(guild_id, channel_override_none) is False
    )  # Inherits global False
    assert (
        kev_db.get_effective_verbosity(guild_id, channel_no_config) is False
    )  # Inherits global False
    ...
    assert kev_db.get_effective_verbosity(guild_id, channel_override_false) is False
    assert (
        kev_db.get_effective_verbosity(guild_id, channel_override_none) is True
    )  # Inherits global True
    assert (
        kev_db.get_effective_verbosity(guild_id, channel_no_config) is True
    )  # Inherits global True
    ...
    assert (
        kev_db.get_effective_verbosity(guild_id_no_global, 201) is True
    )  # Override works
    assert (
        kev_db.get_effective_verbosity(guild_id_no_global, 202) is False
    )  # Inherits default False
    assert (
        kev_db.get_effective_verbosity(guild_id_no_global, 203) is False
    )  # No channel, inherits default False

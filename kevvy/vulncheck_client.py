...
class VulnCheckClient:
    ...
    def __init__(self, api_key: Optional[str]):
        if not api_key:
            logger.warning(
                "VulnCheck API Key not provided. VulnCheck client will not function."
            )
    ...
    def _parse_cvss_from_data(
        ...
        if hasattr(vc_data, "metrics") and vc_data.metrics is not None:
            if (
                hasattr(vc_data.metrics, "cvss_metric_v31")
                and vc_data.metrics.cvss_metric_v31
            ):
                ...
                hasattr(vc_data.metrics, "cvss_metric_v2")
                and vc_data.metrics.cvss_metric_v2
            ):
            ...
            for weakness in vc_data.weaknesses:
                if hasattr(weakness, "description") and weakness.description:
                    for desc in weakness.description:
                        ...
                        if (
                            getattr(desc, "lang", "") == "en"
                            and desc_value
                            and "CWE-" in desc_value
                        ):
                            try:
                                parts = desc_value.split("CWE-", 1)[1]
                                cwe_num = (
                                    parts.split()[0].split("<")[0].split(")")[0].strip()
                                )
                                if cwe_num.isdigit():
                                    cwe_ids.append(f"CWE-{cwe_num}")
                            except IndexError:
                                logger.warning(
                                    f"Could not parse CWE from description: {desc_value}"
                                )
    ...
    def _parse_references_from_data(self, vc_data: Any) -> List[Dict[str, Any]]:
        ...
        if hasattr(vc_data, "references") and vc_data.references:
            for ref in vc_data.references:
                ...
                looks_like_uuid = (
                    len(original_source) == 36 and original_source.count("-") == 4
                )

                if (
                    original_source
                    and "@" not in original_source
                    and not looks_like_uuid
                ):
    ...
    async def get_cve_details(self, cve_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            api_response = await self._run_sdk_call(
                self.indices_client.index_nist_nvd2_get, cve=cve_id
            )

            if not api_response or not api_response.data:
                logger.warning(
                    f"No vulnerability data found for {cve_id} using VulnCheck's NIST NVD2 endpoint."
                )
                return None
            ...
            logger.info(
                f"Successfully fetched details for {cve_from_data} from VulnCheck."
            )
            ...
            logger.error(
                f"VulnCheck API error fetching {cve_id}: {e.status} {e.reason} - {e.body}"
            )
            return None
        except Exception as e:
            logger.error(
                f"An unexpected error occurred processing {cve_id} with VulnCheck: {e}",
                exc_info=True,
            )
            return None

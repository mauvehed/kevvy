...
class DiagnosticsCog(commands.Cog):
    ...
    async def update_web_status(self):
        ...
        try:
            ...
            stats_data['kev_api_errors'] = getattr(self.bot, 'stats_api_errors_kev', 0) # From KEV cog error
            ...
            stats_data['api_errors_vulncheck'] = getattr(self.bot, 'stats_api_errors_vulncheck', 0)
            stats_data['api_errors_cisa'] = getattr(self.bot, 'stats_api_errors_cisa', 0) # From KEV feed check task
            stats_data['rate_limits_nvd'] = getattr(self.bot, 'stats_rate_limits_hit_nvd', 0)
            ...
            stats_data['kev_alerts'] = getattr(self.bot, 'stats_kev_alerts_sent', 0) # Renamed key for server.js
            stats_data['messages_processed'] = getattr(self.bot, 'stats_messages_processed', 0) # Added key for server.js
            stats_data['vulncheck_success'] = getattr(self.bot, 'stats_vulncheck_success', 0)
            stats_data['nvd_fallback_success'] = getattr(self.bot, 'stats_nvd_fallback_success', 0)
                ...
                async with self.session.post(full_api_url, json=stats_data, headers=headers, timeout=request_timeout) as response:
                    if response.status == 200:
                        ...
                        try:
                            ...
                            logger.warning(f"Web API Response Body (Truncated): {error_text[:500]}") # Limit length
                        except Exception as read_err:
                            logger.warning(f"Could not read response body after API error: {read_err}")
            else:

...
class KEVCog(commands.Cog):
    ...
    # --- Root KEV Group ---
    kev_group = app_commands.Group(
        name="kev",
        description="Manage CISA KEV monitoring and lookups.",
        guild_only=True,
    )
    ...
    feed_group = app_commands.Group(
        name="feed",
        parent=kev_group,
        description="Manage KEV feed monitoring for this server.",
    )
    ...
    # --- /kev feed commands ---
    @feed_group.command(
        name="enable", description="Enable KEV feed alerts in the specified channel."
    )
    @app_commands.checks.has_permissions(manage_guild=True)
    async def kev_feed_enable_command(
        self, interaction: discord.Interaction, channel: discord.TextChannel
    ):  # type: ignore
        """Enable CISA KEV monitoring alerts in the specified channel for this server."""
        if not self.db:
            await interaction.response.send_message(
                "Database is not available. Cannot enable KEV monitoring.",
                ephemeral=True,
            )
            return
        ...
        try:
            if interaction.guild_id is None:
                await interaction.response.send_message(
                    "Could not determine server ID.", ephemeral=True
                )
                return
            self.db.set_kev_config(interaction.guild_id, channel.id)
            await interaction.response.send_message(
                f"✅ KEV feed monitoring enabled. Alerts will be sent to {channel.mention}.",
                ephemeral=True,
            )
        except Exception as e:
            logger.error(
                f"Error enabling KEV feed for guild {interaction.guild_id}: {e}",
                exc_info=True,
            )
            if not interaction.response.is_done():
                await interaction.response.send_message(
                    "An error occurred while enabling KEV feed monitoring.",
                    ephemeral=True,
                )

    @feed_group.command(
        name="disable", description="Disable KEV feed alerts for this server."
    )
    @app_commands.checks.has_permissions(manage_guild=True)
    async def kev_feed_disable_command(self, interaction: discord.Interaction):  # type: ignore
        ...
        if not self.db:
            await interaction.response.send_message(
                "Database is not available. Cannot disable KEV monitoring.",
                ephemeral=True,
            )
            return
        ...
        try:
            ...
            logger.error(
                f"Error disabling KEV feed for guild {interaction.guild_id}: {e}",
                exc_info=True,
            )
            if not interaction.response.is_done():
                await interaction.response.send_message(
                    "An error occurred while disabling KEV feed monitoring.",
                    ephemeral=True,
                )

    @feed_group.command(
        name="status", description="Check KEV feed alert status for this server."
    )
    @app_commands.checks.has_permissions(manage_guild=True)
    async def kev_feed_status_command(self, interaction: discord.Interaction):  # type: ignore
        ...
        if not self.db:
            await interaction.followup.send(
                "Database is not available. Cannot check KEV status.", ephemeral=True
            )
            return
        ...
        if config and config["enabled"]:
            channel_mention = (
                f"ID: {config['channel_id']} (Channel not found or inaccessible?)"
            )
    ...
    # --- /kev latest command ---
    @kev_group.command(
        name="latest", description="Display the most recent KEV entries."
    )
    ...
    async def kev_latest_command(  # type: ignore
        ...
        if not self.kev_client:
            await interaction.response.send_message(
                "❌ KEV client is not available. Cannot fetch latest entries.",
                ephemeral=True,
            )
            return
        ...
        try:
            ...
            if self.db and interaction.guild_id:
                ...
                self.db.log_kev_latest_query(
                    interaction.guild_id, interaction.user.id, query_params
                )
            ...
            if not all_kevs:
                await interaction.followup.send(
                    f"⚪ No KEV entries found matching your criteria in the last {days} days.",
                    ephemeral=True,
                )
                return
                ...
                if isinstance(date_added_str, str):
                    try:
                        ...
                        if parsed_date.tzinfo is None:
                            entry_date = parsed_date.replace(
                                tzinfo=datetime.timezone.utc
                            )
                            logger.debug(
                                f"Made naive date {parsed_date} aware for KEV entry {k.get('cveID')}"
                            )
                        else:
                        ...
                        logger.warning(
                            f"Could not parse dateAdded '{date_added_str}' for KEV entry {k.get('cveID')}"
                        )
                else:
                    logger.warning(
                        f"Missing or invalid dateAdded for KEV entry {k.get('cveID')}"
                    )
            ...
            if vendor:
                recent_kevs = [
                    k
                    for k in recent_kevs
                    if vendor.lower() in k.get("vendorProject", "").lower()
                ]
            if product:
                recent_kevs = [
                    k
                    for k in recent_kevs
                    if product.lower() in k.get("product", "").lower()
                ]
            ...
            for i, kev in enumerate(results_to_show, 1):
                ...
                if (
                    i >= 5 and len(results_to_show) > 5
                ):  # Stop adding details after 5 if many results
                    description_lines.append(
            ...
            embed.set_footer(
                text=f"Found {len(recent_kevs)} entries matching criteria. Showing top {len(results_to_show)}."
            )
            ...
            logger.error(
                f"Error fetching KEV catalog for /kev latest: {fetch_err}",
                exc_info=True,
            )
            ...
            # ------------------------------------
            await interaction.followup.send(
                "❌ An error occurred while fetching the KEV catalog data.",
                ephemeral=True,
            )
            return

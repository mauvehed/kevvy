...
class KEVConfigDB:
    ...
    def _initialize_db(self):
        ...
        try:
            ...
            if "enabled" not in kev_columns:
                ...
                cursor.execute(
                    "ALTER TABLE kev_config ADD COLUMN enabled BOOLEAN NOT NULL DEFAULT 0"
                )
            ...
            try:
                ...
                cursor.execute(
                    "SELECT name FROM sqlite_master WHERE type='table' AND name='cve_channel_config'"
                )
                old_table_exists = cursor.fetchone() is not None
                cursor.execute(
                    "SELECT name FROM sqlite_master WHERE type='table' AND name='cve_guild_config'"
                )
                new_table_exists = cursor.fetchone() is not None

                if old_table_exists and not new_table_exists:
                    logger.warning(
                        "Found old 'cve_channel_config' table and no 'cve_guild_config'. Attempting rename and column adjustments."
                    )
                    # Rename table
                    cursor.execute(
                        "ALTER TABLE cve_channel_config RENAME TO cve_guild_config"
                    )
                    ...
                    if "enabled" not in guild_config_columns:
                        cursor.execute(
                            "ALTER TABLE cve_guild_config ADD COLUMN enabled BOOLEAN DEFAULT true"
                        )
                    if "verbose_mode" not in guild_config_columns:
                        cursor.execute(
                            "ALTER TABLE cve_guild_config ADD COLUMN verbose_mode BOOLEAN DEFAULT false"
                        )
                    if "severity_threshold" not in guild_config_columns:
                        cursor.execute(
                            "ALTER TABLE cve_guild_config ADD COLUMN severity_threshold TEXT DEFAULT 'all'"
                        )
                    if "last_updated" not in guild_config_columns:
                        cursor.execute(
                            "ALTER TABLE cve_guild_config ADD COLUMN last_updated TIMESTAMP DEFAULT CURRENT_TIMESTAMP"
                        )

                    logger.info(
                        "Successfully renamed 'cve_channel_config' to 'cve_guild_config' and checked columns."
                    )

                elif old_table_exists and new_table_exists:
                    logger.warning(
                        "Both 'cve_channel_config' and 'cve_guild_config' tables exist. Skipping rename. The old table may need manual review or deletion."
                    )
                ...
                logger.error(
                    f"Error during cve_channel_config migration check/rename: {mig_err}",
                    exc_info=True,
                )
            ...
            try:
                ...
                logger.error(
                    f"Error ensuring columns exist in cve_guild_config: {add_col_err}",
                    exc_info=True,
                )
            ...
            try:
                ...
                channel_configs_columns = {
                    column["name"] for column in cursor.fetchall()
                }
                if "enabled" not in channel_configs_columns:
                    cursor.execute(
                        "ALTER TABLE cve_channel_configs ADD COLUMN enabled BOOLEAN DEFAULT true"
                    )
                if "verbose_mode" not in channel_configs_columns:
                    cursor.execute(
                        "ALTER TABLE cve_channel_configs ADD COLUMN verbose_mode BOOLEAN DEFAULT NULL"
                    )
                if "severity_threshold" not in channel_configs_columns:
                    cursor.execute(
                        "ALTER TABLE cve_channel_configs ADD COLUMN severity_threshold TEXT DEFAULT NULL"
                    )
                if "alert_format" not in channel_configs_columns:
                    cursor.execute(
                        "ALTER TABLE cve_channel_configs ADD COLUMN alert_format TEXT DEFAULT NULL"
                    )
                if "created_at" not in channel_configs_columns:
                    cursor.execute(
                        "ALTER TABLE cve_channel_configs ADD COLUMN created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP"
                    )
                if "updated_at" not in channel_configs_columns:
                    cursor.execute(
                        "ALTER TABLE cve_channel_configs ADD COLUMN updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP"
                    )
                ...
                logger.error(
                    f"Error ensuring columns exist in cve_channel_configs: {add_col_err_ch}",
                    exc_info=True,
                )
    ...
    def set_kev_config(self, guild_id: int, channel_id: int):
        ...
        try:
            ...
            logger.info(
                f"Enabled/Updated KEV config for guild {guild_id} to channel {channel_id}."
            )
        except sqlite3.Error as e:
            logger.error(
                f"Error setting KEV config for guild {guild_id}: {e}", exc_info=True
            )
    ...
    def disable_kev_config(self, guild_id: int):
        ...
        try:
            ...
            if cursor.rowcount > 0:
                ...
                logger.info(
                    f"Attempted to disable KEV config for guild {guild_id}, but it was not configured."
                )
        except sqlite3.Error as e:
            logger.error(
                f"Error disabling KEV config for guild {guild_id}: {e}", exc_info=True
            )
    ...
    def get_kev_config(self, guild_id: int) -> Optional[Dict]:
        ...
        try:
            cursor = self._conn.cursor()
            cursor.execute(
                "SELECT guild_id, channel_id, enabled FROM kev_config WHERE guild_id = ?",
                (guild_id,),
            )
            row = cursor.fetchone()
            ...
            logger.error(
                f"Error getting KEV config for guild {guild_id}: {e}", exc_info=True
            )
            return None
    ...
    def get_enabled_kev_configs(self) -> List[Dict]:
        ...
        try:
            cursor = self._conn.cursor()
            cursor.execute(
                "SELECT guild_id, channel_id FROM kev_config WHERE enabled = 1"
            )
    ...
    def add_seen_kevs(self, cve_ids: set[str]):
        ...
        try:
            ...
            cursor.executemany(
                "INSERT OR IGNORE INTO seen_kevs (cve_id) VALUES (?)", data_to_insert
            )
            logger.debug(
                f"Attempted to add {len(cve_ids)} KEV IDs to seen list in DB (ignored duplicates)."
            )
        except sqlite3.Error as e:
    ...
    def count_globally_enabled_cve_guilds(self) -> int:
        """Counts the number of distinct guilds with global CVE monitoring enabled."""
        if not self._conn:
            logger.error(
                "Cannot count enabled CVE guilds: Database connection is not available."
            )
            return 0
        try:
            cursor = self._conn.cursor()
            cursor.execute(
                """
                SELECT COUNT(DISTINCT guild_id) FROM cve_guild_config WHERE enabled = 1
            """
            )
            ...
            logger.error(
                f"Database error counting globally enabled CVE guilds: {e}",
                exc_info=True,
            )
            return 0

    def count_active_cve_channels(self) -> int:
        """Counts the number of distinct channels with specific CVE monitoring enabled."""
        # This counts rows in cve_channel_configs where enabled is explicitly true.
        # It doesn't consider the global guild setting.
        if not self._conn:
            logger.error(
                "Cannot count active CVE channels: Database connection is not available."
            )
            return 0
        try:
            cursor = self._conn.cursor()
            # We count distinct guild_id, channel_id pairs where enabled is true in the channel config table
            # Note: The 'enabled' column in cve_channel_configs might not be strictly necessary
            # if the presence of a row implies configuration, but we'll use it for now.
            cursor.execute(
                """
                SELECT COUNT(DISTINCT channel_id) FROM cve_channel_configs WHERE enabled = 1
            """
            )
            result = cursor.fetchone()
            return result[0] if result else 0
            ...
            logger.error(
                f"Database error counting active CVE channels: {e}", exc_info=True
            )
            return 0
    ...
    def set_cve_guild_config(
        ...
        try:
            ...
            logger.error(
                f"Error setting global CVE config for guild {guild_id}: {e}",
                exc_info=True,
            )
    ...
    def get_cve_guild_config(self, guild_id: int) -> Optional[Dict]:
        ...
        try:
            ...
            logger.error(
                f"Error getting CVE guild config for guild {guild_id}: {e}",
                exc_info=True,
            )
            return None
    ...
    def update_cve_guild_enabled(self, guild_id: int, enabled: bool):
        ...
        try:
            ...
            logger.info(
                f"Updated global CVE enabled status for guild {guild_id} to {enabled}."
            )
        except sqlite3.Error as e:
            logger.error(
                f"Error updating CVE guild enabled status for {guild_id}: {e}",
                exc_info=True,
            )
    ...
    def update_cve_guild_verbose_mode(self, guild_id: int, verbose_mode: bool):
        ...
        try:
            ...
            logger.info(
                f"Set/Updated global CVE verbose mode for guild {guild_id} to {verbose_mode}."
            )
        except sqlite3.Error as e:
            logger.error(
                f"Error setting global CVE verbose mode for {guild_id}: {e}",
                exc_info=True,
            )

    def update_cve_guild_severity_threshold(
        self, guild_id: int, threshold: SeverityLevel
    ):
        ...
        try:
            ...
            logger.info(
                f"Set/Updated global CVE severity threshold for guild {guild_id} to {threshold}."
            )
        except sqlite3.Error as e:
            logger.error(
                f"Error setting global CVE severity threshold for {guild_id}: {e}",
                exc_info=True,
            )

    # --- NEW Methods for Per-Channel CVE Configs ---

    def add_or_update_cve_channel(
        self,
        guild_id: int,
        channel_id: int,
        enabled: bool = True,
        ...
        alert_format: Optional[str] = None,
    ):
        ...
        try:
            ...
            logger.info(
                f"Added/Updated CVE config for channel {channel_id} in guild {guild_id}."
            )
        except sqlite3.Error as e:
            logger.error(
                f"Error adding/updating CVE channel config for {channel_id} in {guild_id}: {e}",
                exc_info=True,
            )
    ...
    def remove_cve_channel(self, guild_id: int, channel_id: int):
        ...
        try:
            cursor = self._conn.cursor()
            cursor.execute(
                "DELETE FROM cve_channel_configs WHERE guild_id = ? AND channel_id = ?",
                (guild_id, channel_id),
            )
            logger.info(
                f"Removed CVE config for channel {channel_id} in guild {guild_id}."
            )
        except sqlite3.Error as e:
            logger.error(
                f"Error removing CVE channel config for {channel_id} in {guild_id}: {e}",
                exc_info=True,
            )
    ...
    def get_cve_channel_config(self, guild_id: int, channel_id: int) -> Optional[Dict]:
        ...
        try:
            ...
            logger.error(
                f"Error getting CVE channel config for {channel_id} in {guild_id}: {e}",
                exc_info=True,
            )
            return None
    ...
    def get_all_cve_channel_configs_for_guild(self, guild_id: int) -> List[Dict]:
        ...
        try:
            ...
            logger.error(
                f"Error getting all CVE channel configs for guild {guild_id}: {e}",
                exc_info=True,
            )
            return []

    def set_channel_verbosity(
        self, guild_id: int, channel_id: int, verbose_mode: Optional[bool]
    ):
        ...
        try:
            ...
            status = (
                "set to " + str(verbose_mode)
                if verbose_mode is not None
                else "unset (use global)"
            )
            logger.info(
                f"Verbosity for channel {channel_id} in guild {guild_id} {status}."
            )
        except sqlite3.Error as e:
            logger.error(
                f"Error setting verbosity for channel {channel_id} in {guild_id}: {e}",
                exc_info=True,
            )
    ...
    def set_all_channel_verbosity(self, guild_id: int, verbose_mode: bool):
        ...
        try:
            ...
            logger.info(
                f"Set verbosity for ALL channels in guild {guild_id} to {verbose_mode}."
            )
        except sqlite3.Error as e:
            logger.error(
                f"Error setting verbosity for all channels in guild {guild_id}: {e}",
                exc_info=True,
            )
            raise
    ...
    def delete_all_cve_channel_configs(self, guild_id: int) -> int:
        ...
        try:
            cursor = self._conn.cursor()
            cursor.execute(
                "DELETE FROM cve_channel_configs WHERE guild_id = ?", (guild_id,)
            )
            deleted_count = cursor.rowcount
            logger.info(
                f"Deleted {deleted_count} CVE channel configs for guild {guild_id}."
            )
            return deleted_count
        except sqlite3.Error as e:
            logger.error(
                f"Error deleting CVE channel configs for guild {guild_id}: {e}",
                exc_info=True,
            )
    ...
    def get_effective_verbosity(self, guild_id: int, channel_id: int) -> bool:
        ...
        return (
            bool(guild_config["verbose_mode"])
            if guild_config and "verbose_mode" in guild_config
            else False
        )
    ...
    def log_cve_alert_history(self, guild_id: int, channel_id: int, cve_id: str):
        ...
        try:
            ...
            logger.debug(
                f"Logged CVE {cve_id} alert history for guild {guild_id} in channel {channel_id}."
            )
        except sqlite3.Error as e:
            logger.error(
                f"Error logging CVE alert history for guild {guild_id}: {e}",
                exc_info=True,
            )

    def log_kev_latest_query(
        self, guild_id: int, user_id: int, query_params: Dict[str, Any]
    ):
        ...
        try:
            ...
            logger.debug(
                f"Logged /kev latest query for user {user_id} in guild {guild_id}."
            )
        except sqlite3.Error as e:
            logger.error(
                f"Error logging /kev latest query for guild {guild_id}: {e}",
                exc_info=True,
            )
        except json.JSONDecodeError as e:
            logger.error(
                f"Error serializing query params to JSON for KEV latest query log: {e}",
                exc_info=True,
            )

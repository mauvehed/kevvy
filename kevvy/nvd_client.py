...
class NVDClient:
        ...
        try:
            ...
            async with self.session.get(
                ...
                try:
                    ...
                    logger.warning(
                        f"Non-JSON response received from NVD (Status: {status})."
                    )
            ...
            logger.error(
                f"Unexpected error during GET execution for NVD: {e}", exc_info=True
            )
    ...
    async def _make_request(self, params: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        while retries <= self.MAX_RETRIES:
            ...
            try:
                ...
                if status_code == 200:
                    ...
                    retries += 1
                    logger.warning(
                        f"NVD API retryable error (status {status_code}). Retrying in {self.retry_delay}s... ({retries}/{self.MAX_RETRIES})"
                    )
                    ...
                    if status_code in [403, 429]:
                        raise NVDRateLimitError(
                            f"NVD API rate limit hit (Status: {status_code})"
                        )
                ...
                logger.error(
                    f"An unexpected error occurred during NVD request processing: {e}",
                    exc_info=True,
                )
        ...
        if last_status_code in [403, 429]:
            raise NVDRateLimitError(
                f"NVD API rate limit hit after max retries (Last Status: {last_status_code})"
            )
        return None
    ...
    async def get_cve_details(self, cve_id: str) -> Dict[str, Any] | None:
        ...
        try:
            ...
            if data and data.get("vulnerabilities"):
                ...
                logger.warning(
                    f"NVD API returned data for {cve_id}, but no 'vulnerabilities' key found."
                )
                return None
            else:
            ...
            logger.error(
                f"Unexpected error in get_cve_details for {cve_id}: {e}", exc_info=True
            )
            return None

    # --- NEW Method: get_recent_cves ---
    async def get_recent_cves(
        self, days: int = DEFAULT_RECENT_DAYS
        ...
        while True:
            ...
            try:
                ...
                if total_results == -1:  # First request
                    ...
                    if total_results == 0:
                        logger.info(
                            f"No CVEs found published between {start_date_str} and {end_date_str}."
                        )
                        return []
                ...
                if not vulnerabilities:
                    logger.warning(
                        f"NVD response for recent CVEs had no vulnerabilities list (startIndex: {start_index})."
                    )
                    break  # Exit loop if no vulnerabilities returned

                logger.info(
                    f"Fetched batch of {len(vulnerabilities)} CVEs (startIndex: {start_index}, total: {total_results})."
                )
                ...
                if len(vulnerabilities) < self.MAX_RESULTS_PER_PAGE:
                    logger.warning(
                        "NVD returned fewer results than requested per page, stopping pagination early."
                    )
                ...
                logger.error(
                    f"Unexpected error fetching recent CVEs batch: {e}", exc_info=True
                )
                # Return what we have so far or None
                return all_parsed_cves or None

        logger.info(
            f"Finished fetching recent CVEs. Total parsed: {len(all_parsed_cves)}"
        )
        return all_parsed_cves

    def _parse_cve_data(
        self, cve_item: Dict[str, Any], url: str
    ) -> Dict[str, Any] | None:
        ...
        if cvss_score is not None and cvss_version:
            ...
            if metrics_data.get("cvssMetricV31"):
                severity = (
                    metrics_data["cvssMetricV31"][0]
                    .get("cvssData", {})
                    .get("baseSeverity")
                )
            elif metrics_data.get("cvssMetricV30"):
                severity = (
                    metrics_data["cvssMetricV30"][0]
                    .get("cvssData", {})
                    .get("baseSeverity")
                )
            elif metrics_data.get("cvssMetricV2"):
